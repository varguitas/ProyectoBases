...
CREATE PROCEDURE get_entrenadores_no_inscritos(@ID_TORNEO int) AS
	BEGIN
		SELECT ID_ENTRENADOR, (SELECT NOMBRE FROM PAIS WHERE PAIS.ID_PAIS=ENTRENADOR.ID_PAIS) AS PAIS, CONCAT(NOMBRE,' ',APELLIDOS) AS NOMBRE, FECHA_NACIMIENTO
		FROM ENTRENADOR
		WHERE ID_ENTRENADOR NOT IN (SELECT ee.ID_ENTRENADOR FROM ENTRENADOR_EQUIPO ee inner join INSCRIPCION_EQUIPO ie ON ee.ID_INSCRIPCION_EQUIPO = ie.ID_INSCRIPCION_EQUIPO inner join TORNEO t on t.ID_TORNEO = ie.ID_TORNEO WHERE t.ID_TORNEO = @ID_TORNEO)
		ORDER BY NOMBRE ASC
	END
GO
