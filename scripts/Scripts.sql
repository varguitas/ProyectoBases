...
--GET_PROGRAMACION
if object_id('add_alineacion') is not null
begin
	DROP PROCEDURE add_alineacion
end
GO
CREATE PROCEDURE add_alineacion(@ID_PARTIDO int, @IDS varchar(max)) AS
	BEGIN
		DECLARE @xml_ids XML = @IDS
		CREATE TABLE #jugadores (
	...
	END
GO
GRANT EXEC ON add_alineacion TO adm_user;
GO
...
GO
CREATE PROCEDURE get_alineacion
	@ID_INSCRIPCION_EQUIPO INT,
	@ID_PARTIDO INT
AS BEGIN
		SELECT A.ID_ALINEACION, CONCAT(J.NOMBRE,' ',J.APELLIDOS), A.TITULAR, A.NUMERO_CAMISETA
		FROM ALINEACION A
			INNER JOIN INSCRIPCION_JUGADOR IJ
			ON IJ.ID_INSCRIPCION_JUGADOR=A.ID_INSCRIPCION_JUGADOR
			INNER JOIN JUGADOR J
			ON IJ.ID_JUGADOR=J.ID_JUGADOR
		WHERE IJ.ID_INSCRIPCION_EQUIPO=@ID_INSCRIPCION_EQUIPO AND A.ID_PARTIDO=@ID_PARTIDO
	END
GO
...
begin
	DROP PROCEDURE contar_jugadores_formacion
end
GO
CREATE PROCEDURE contar_jugadores_formacion (@ID_PARTIDO int, @ID_INSCRIPCION_EQUIPO int) 
AS
	BEGIN
		DECLARE @JUGADORES int =
		(SELECT count(*)
		FROM (SELECT ID_INSCRIPCION_JUGADOR FROM ALINEACION WHERE ID_PARTIDO=@ID_PARTIDO) aj
			inner join INSCRIPCION_JUGADOR ij
			on (aj.ID_INSCRIPCION_JUGADOR=ij.ID_INSCRIPCION_JUGADOR)
			inner join (SELECT ID_INSCRIPCION_EQUIPO FROM INSCRIPCION_EQUIPO WHERE ID_INSCRIPCION_EQUIPO = @ID_INSCRIPCION_EQUIPO) ie
			on (ij.ID_INSCRIPCION_EQUIPO = ie.ID_INSCRIPCION_EQUIPO))
...
if object_id('get_jugadores_inscritos') is not null
begin
	DROP PROCEDURE get_jugadores_inscritos
end
GO
CREATE PROCEDURE get_jugadores_inscritos
	@ID_INSCRIPCION_EQUIPO INT
AS BEGIN
	SELECT IJ.ID_INSCRIPCION_JUGADOR,J.ID_JUGADOR, CONCAT(J.NOMBRE,' ',J.APELLIDOS) AS NOMBRE, P.DESCRIPCION AS POSICION
	FROM JUGADOR J 
		INNER JOIN INSCRIPCION_JUGADOR IJ
		ON J.ID_JUGADOR= IJ.ID_JUGADOR
		INNER JOIN POSICION P
		ON IJ.ID_POSICION = P.ID_POSICION
	WHERE IJ.ID_INSCRIPCION_EQUIPO=@ID_INSCRIPCION_EQUIPO
END
GO
GRANT EXEC ON get_jugadores_inscritos TO adm_user;
GO
